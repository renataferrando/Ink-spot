...
export default async function ArtistOgImage({ params }: Props) {
  ...
  try {
    ...
    if (res.ok) {
      const json = await res.json() as { artist?: { display_name?: string; current_location?: { location_name?: string }; primary_styles?: string[] } };
      if (json.artist) {
  ...
  return new ImageResponse(
    (
      ...
      >
        <div style={{ fontSize: 14, color: "#6b7280", letterSpacing: "3px", textTransform: "uppercase" }}>
          InkSpot
        </div>
        <div style={{ fontSize: 56, fontWeight: 700, color: "#f9fafb", lineHeight: 1.1 }}>
          {name}
        </div>
